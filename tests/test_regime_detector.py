...
from carcharoth.regime import Regime, RegimeDetector, build_feature
...
def test_stability_defaults_to_one_without_change_features() -> None:
    detector = RegimeDetector(features=[(StubFeature("a", direction=-0.4), 1.0)], lookback=100)
    assessment = detector.assess("AAPL", BARS)
    assert assessment is not None
...
def test_zero_score_falls_to_mean_reversion() -> None:
    detector = RegimeDetector(features=[(StubFeature("a", direction=0.0), 1.0)], lookback=100)
    assessment = detector.assess("AAPL", BARS)
    assert assessment is not None
    assert assessment.regime is Regime.MEAN_REVERTING
...
def test_no_directional_evidence_returns_none() -> None:
    detector = RegimeDetector(features=[(StubFeature("a", stability=0.5), 1.0)], lookback=100)
...
def test_too_few_bars_returns_none() -> None:
    detector = RegimeDetector(features=[(StubFeature("a", direction=1.0), 1.0)], lookback=100)
...
def test_assessment_carries_all_evidence() -> None:
    ...
    assert detector.weight_of("a") == 1.0
    assert detector.weight_of("missing") is None


def test_rejects_invalid_construction() -> None:
    with pytest.raises(ValueError):
        RegimeDetector(features=[], lookback=100)
    with pytest.raises(ValueError):
        RegimeDetector(features=[(StubFeature("a", direction=1.0), 0.0)], lookback=100)
