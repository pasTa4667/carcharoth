...
def format_summary(outcome: QuickTestOutcome, objective: str) -> str:
    """Human-readable console summary: aggregate line + per-symbol table."""
    agg = {m.name: m.value for m in outcome.metrics if m.symbol is None}
    fitness_name = fitness_metric_name(objective)
    lines = ["quicktest complete", f"  run_id:  {outcome.run_id}", "", "aggregate:"]
    for name in (
        fitness_name,
        "sharpe",
        "profit_factor",
        ...
        if name in agg:
            lines.append(f"  {name:<20s} {agg[name]:>12.4f}")

    by_symbol: dict[str, dict[str, float]] = {}
    for m in outcome.metrics:
        if m.symbol is not None and m.name != "symbol_pnl":
            by_symbol.setdefault(m.symbol, {})[m.name] = m.value
    if by_symbol:
        columns = (
            "sharpe",
            "profit_factor",
        ...
        header = "  " + f"{'symbol':<8s}" + "".join(f"{c:>15s}" for c in columns)
        lines += ["", "per symbol:", header]
        for symbol in sorted(by_symbol):
            values = by_symbol[symbol]
            cells = "".join(
                f"{values[c]:>15.4f}" if c in values else f"{'-':>15s}" for c in columns
            )
            lines.append(f"  {symbol:<8s}{cells}")
    return "\n".join(lines)
